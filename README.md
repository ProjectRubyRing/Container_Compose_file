...
- front/back は `read_only: true` (ルートFS読み取り専用) でハードニングしている。
  `/mnt/logs` `/mnt/data` は named volume マウントのため read_only の対象外で
  **ディレクトリ作成・ファイル書き込みともに可能** (「マウント先だから書ける」)。
  JBoss EAP が実行時に書き込む `${JBOSS_HOME}/standalone` 配下は、entrypoint が
  ルートFS読み取り専用を検知したときだけ `tmpfs` (`/tmp`) へ複製し `JBOSS_BASE_DIR` を
  そこへ向ける。ルートFSが書き込み可能な環境 (ECS taskdef は `readOnlyRootFilesystem`
  未設定) では何もしないため挙動は変わらない。
