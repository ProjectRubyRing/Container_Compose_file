# =============================================================================
# Lambda 関数 (ローカル代替) — SQS イベントを受け取り、ALB 経由で backend を POST 呼び出し
# ---------------------------------------------------------------------------
# 配置場所:
...
#   この関数は各レコードの body を取り出し、ALB (nginx) 経由で backend の
...
# ALB のリスナールール → ターゲットグループ (backend) へ流すパス。
# backend 側 Java サーブレットのコンテキストルート /async + マッピング /receive。
BACK_PATH = os.environ.get("BACK_PATH", "/async/receive")

# backend への1リクエストのタイムアウト秒数
HTTP_TIMEOUT = float(os.environ.get("HTTP_TIMEOUT_SECONDS", "10"))


def _post_to_back(body: str, message_id: str) -> tuple[int, str]:
    """ALB 経由で backend の Java サーブレットへ POST し、(HTTPステータス, 応答本文) を返す。"""
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "X-Source": "lambda-local",          # backend 側でトレース確認用
    ...
    for record in records:
        ...
        try:
            ...
            if 200 <= status < 300:
                ...
                # backend が 4xx/5xx を返した → このメッセージは失敗扱い (再処理へ)
