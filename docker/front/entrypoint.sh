...
import_trusted_certs() {
  ...
  # Dockerfile の --mount=type=secret ステップが /opt/pki/build-import.txt を書く。
  # 記録があるということは JDK 同梱 cacerts と jboss-truststore.p12 に
  # 焼き込み済みという意味なので、pki volume を付けずに起動しても HTTPS は通る。
